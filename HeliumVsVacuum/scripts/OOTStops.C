void OOTStops() {
  ...
  bool save_plots = false;//true;
  std::string basename = "";

  const int n_hists = 1;
  std::string histnames[n_hists] = {"Zstop"};//, "tgtmut"};
  std::string histtitles[n_hists] = {"Muon OOT Stops (z-position)"};//, "Muon OOT Stops (time)"};
  std::string x_titles[n_hists] = {"Z [mm]"};//, "t [ns]"};
  ...
  TLegend* leg = new TLegend(0.17, 0.65, 0.50, 0.85);
  ...
  for (int i_hist = 0; i_hist < n_hists; ++i_hist) {
    ...
    for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
      ...
      std::string filename = "data/val.OOTStops." + conf + ".root";
      TFile* file = new TFile(filename.c_str(), "READ");
      //      TTree* events = (TTree*) file->Get("Events");
      //      int n_events = events->GetEntries();
      std::string foldername = "Validation/SimParticlemv_OOTStopFilter_noName";
      ...
      h->SetYTitle("Muon Stops / POT / bin");
      ...
      if (i_hist==0) {
        leglabel.str("");
        leglabel << conf << std::fixed << std::setprecision(4) << " (" << n_events/n_POT << " mu-stop / POT)";
        leg->AddEntry(h, leglabel.str().c_str(), "pl");
      }

      double min_z_DS = 3239;
      double max_z_DS = 14139;
      int min_integral_bin = h->GetXaxis()->FindBin(min_z_DS);
      int max_integral_bin = h->GetXaxis()->FindBin(max_z_DS);
      double mu_stop_DS = h->Integral(min_integral_bin, max_integral_bin);
      std::cout << "Stops in DS = " << mu_stop_DS << std::endl;
    ...
    if (save_plots) {
      std::string pdfname = basename + "/Mu2eSim_HeliumVsVacuum_OOTStops_" + histname + ".pdf";
