void TargetStops() {
  ...
  const int n_hists = 2;
  std::string histnames[n_hists] = {"tgtmuz", "tgtmut"};
  std::string histtitles[n_hists] = {"Muon Target Stops (z-position)", "Muon Target Stops (time)"};
  std::string x_titles[n_hists] = {"Z [mm]", "t [ns]"};
  std::string draw_opts[n_hists] = {"P E SAME", "HIST E SAME"};
  ...
  for (int i_hist = 0; i_hist < n_hists; ++i_hist) {
    ...
    for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
      ...
      std::string filename = "data/val.TargetStops." + conf + ".root";
      TFile* file = new TFile(filename.c_str(), "READ");
      //      TTree* events = (TTree*) file->Get("Events");
      //      int n_events = events->GetEntries();
      std::string foldername = "Validation/SimParticlemv_TargetStopFilter_noName";
    ...
    if (save_plots) {
      std::string pdfname = basename + "/Mu2eSim_HeliumVsVacuum_TargetStops_" + histname + ".pdf";
