void MuBeam() {
  ...
  std::string histtitles[n_hists] = {"Particles Entering DS (MuBeam)", "Particles Entering DS (MuBeam)"};
  ...
  double resample_factors[n_settings] = {1, 1, 1};
  ...
  for (int i_hist = 0; i_hist < n_hists; ++i_hist) {
    ...
    for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
      ...
      std::string filename = "data/val.MuBeamCat." + conf + ".root";
      ...
      if (i_hist==0) {
        leglabel.str("");
        leglabel << conf << std::fixed << std::setprecision(2) << " (" << n_events/n_POT/1e-3 << " #times 10^{-3} particles / POT)";
    ...
    if (save_plots) {
      std::string pdfname = basename + "/Mu2eSim_HeliumVsVacuum_MuBeam_" + histname + ".pdf";
